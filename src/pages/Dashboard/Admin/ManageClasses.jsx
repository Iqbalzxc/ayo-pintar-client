...
const ManageClasses = () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (res.data.modifiedCount > 0) {
          ...
          const updateClasses = classes.map((cls) =>
            cls._id === id
              ? { ...cls, status: "rejected", reason: result.value }
              : cls
          );
          setClasses(updateClasses);
          console.log(res.data);
  ...
  const handleSaveFeedback = async (e) => {
    ...
    try {
      const res = await axiosSecure.patch(
        `/change-status/${feedbackClass._id}`,
        { status: feedbackClass.status, reason }
      );
      if (res.data.modifiedCount > 0) {
        setClasses(
          classes.map((cls) =>
            cls._id === feedbackClass._id ? { ...cls, reason } : cls
          )
        );
        setFeedbackClass(null);
        Swal.fire(
          "Umpan Balik Disimpan!",
  ...
  return (
    <div>
      <h1 className="text-4xl text-secondary font-bold text-center my-10 mt-24 md:mt-8">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-xl">
            <h2 className="text-2xl mb-2">
              Umpan Balik untuk {feedbackClass.name}
            </h2>
